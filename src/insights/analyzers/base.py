...
class BaseAnalyzer(ABC):
    ...
    def __init__(self, name: str, config: InsightsConfig, store: InsightsStore):
        """Initialize analyzer.
    ...
    def is_enabled(self) -> bool:
        ...
        analyzer_config = self.config.analyzers.get(self.name)
        if analyzer_config:
            return analyzer_config.enabled
    ...
    def was_insight_delivered_recently(
        ...
        """
        if hours is None:
            hours = self.config.decision.cooldown_minutes / 60
